...
class PrtNet(LSTM):
    def __init__(self, hidden_shape, *args, **kwargs):
        ...
        self.input_length = []
        super(PrtNet, self).__init__(*args, **kwargs)

    def build(self, input_shape):
        super(PrtNet, self).build(input_shape)
    ...
    def call(self, x, mask=None):
        ...
        constants = super(PrtNet, self).get_constants(x_input)
    ...
    def step(self, x_input, states):
        input_shape = self.input_spec[0].shape
        en_seq = states[-1]
        _, [h, c] = super(PrtNet, self).step(x_input, states[:-1])
